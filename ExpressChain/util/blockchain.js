...
const createNewBlock = (carNoanddata) => {
  const previousBlock = getLastBlock();
  const newBlockIndex = previousBlock.index + 1;
  const newTimestamp = getTimestamp();
  const carHash = getcarNoHash(carNoanddata);
...
const getcarnohashes = (carNoanddata) =>{
  const index = blockchain.carHash.findIndex(getcarNoHash(carNoanddata));
